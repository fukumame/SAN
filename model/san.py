...
def position(H, W, is_cuda=True):
    if is_cuda:
        ...
        loc_h = torch.linspace(-1.0, 1.0, H).cuda() # -1から1の間でHeightの分だけステップ変化させる (loc_h: H)
...
class SAM(nn.Module):
    ...
    def forward(self, x):
        ...
        # x3は論文中のEq2における、βを表し、x1とx2はAttentionを計算する関数αの引数(x_i, x_j)を表している
        x1, x2, x3 = self.conv1(x), self.conv2(x), self.conv3(x) # x1, x2, x3: bs x C x H x W

        if self.sa_type == 0:  # ペアごとのSelf-Attentionの処理
            # 位置エンコーディングの計算処理
            ...
            if self.stride != 1:
                x1 = self.unfold_i(x1)
            bs, _, h, w = x.shape
            x1 = x1.view(bs, -1, h*w) # x1: bs x C x (H*W)

            # 後段で2Dの畳み込み処理を掛けるために、次元を1つ増やす
            # 高さ1、横幅H*Wの画像のイメージ
            x1 = x1.unsqueeze(dim=2) # x1: bs x C x 1 x (H*W)

            # パッチのサイズが全ピクセルにおいて等しくなるように、padding処理を施す
            # このpadding処理を入れないと、両端のパッチのサイズが1ピクセル分、欠けてしまう
            x2 = self.pad(x2) # x2: bs x C x ((H+1)*(W+1))

            # unfold処理は、コンストラクタの引数にて指定された、kernel_sizeの単位(ここでは3x3)で1pixelずつスライドさせながら、tensorを切り出す処理
            # つまり、対象ピクセルと3x3の周囲ピクセルの関係性を計算するため、周囲ピクセルのテンソルを1pxずつスライドさせながら、切り出している
            # unfold処理の具体的なイメージはこちら (https://blog.shikoan.com/pytorch-extract-patches/#torch_Tensor_unfold) が分かりやすい
            x2 = self.unfold_j(x2) # x2: bs x C*9 x (H*W)

            # x1と階数をあわせておく
            x2 = x2.view(x.shape[0], -1, 1, x1.shape[-1]) # x2: bs x C*9 x 1 x (H*W)

            # この処理が重要なx_iとその周辺ピクセルx_jの関係性を定義する関係関数
            # これは論文中のEq5におけるδを表している
            # このケースでは、Concatenationの関係関数を用いている
...
if __name__ == '__main__':
    # テスト用の実行コード
    net = san(sa_type=1, layers=(3, 4, 6, 8, 3), kernels=[3, 7, 7, 7, 7], num_classes=1000).cuda().eval()
    print(net)
